...
def load_de_vocab(hp):
    vocab = [line.split()[0] for line in open(base_dir+'preprocessed/de.vocab.tsv', 'r', 'utf-8').read().splitlines() if int(line.split()[1])>=hp.min_cnt]
    word2idx = {word: idx for idx, word in enumerate(vocab)}
    idx2word = {idx: word for idx, word in enumerate(vocab)}
    return word2idx, idx2word

def load_en_vocab(hp):
    vocab = [line.split()[0] for line in open(base_dir+'preprocessed/en.vocab.tsv', 'r', 'utf-8').read().splitlines() if int(line.split()[1])>=hp.min_cnt]
    word2idx = {word: idx for idx, word in enumerate(vocab)}
    idx2word = {idx: word for idx, word in enumerate(vocab)}
    return word2idx, idx2word

def load_speaker_vocab(hp):
    vocab = [line.split('\n')[0] for line in open(base_dir+'preprocessed/speakers.txt', 'r', 'utf-8').read().splitlines()]
    word2idx = {word: idx for idx, word in enumerate(vocab)}
    idx2word = {idx: word for idx, word in enumerate(vocab)}
    return word2idx, idx2word
...
def create_data(hp, source_sents, target_sents, image_fea, A): 
    ...
    max_turn = 0
    max_length = 0
    # print("len source_sents:", len(source_sents))
    # print("len target_sents:", len(target_sents))
    # print("len image_fea_sents:", len(image_fea))
    # print("len A:", len(A))
...
def load_image_data(hp, data_type):
    image_features = [line for line in open(hp.corpora_path + data_type+'_image.txt', 'r', 'utf-8').read().split("\n") if line]

    return image_features

def load_train_data(hp):
    de_sents = [line for line in open(hp.source_train, 'r', 'utf-8').read().split("\n") if line]
    en_sents = [line for line in open(hp.target_train, 'r', 'utf-8').read().split("\n") if line]
...
def load_test_data(hp):
    ...
    de_sents = [_refine(line) for line in open(hp.source_test, 'r', 'utf-8').read().split("\n") if line]
    en_sents = [_refine(line) for line in open(hp.target_test, 'r', 'utf-8').read().split("\n") if line]
...
def load_dev_data(hp):
    ...
    de_sents = [_refine(line) for line in open(hp.source_dev, 'r', 'utf-8').read().split("\n") if line]
    en_sents = [_refine(line) for line in open(hp.target_dev, 'r', 'utf-8').read().split("\n") if line]
    image_fea = load_image_data(hp, data_type='dev')
    A = pkl.load(open(hp.corpora_path + 'dev.pkl', 'rb'))
    X, X_image, Y_image, X_length, Y, Sources, Targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A = create_data(hp, de_sents, en_sents, image_fea, A)
    return X, X_image, Y_image, X_length,Y, Sources, Targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A # (1064, 150)

def get_dataset(hp, dataset):
    # Load data
    if dataset=='train':
...
if __name__ == '__main__':
    ...
    for i, (X, X_image, Y_image, X_length, Y, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A) in enumerate(validation_dataset):
        inspectable = 5
        print(X[inspectable])
        print(X_image[inspectable])
        print(Y_image[inspectable])
        print(Y[inspectable])
        print(TGT_emotion[inspectable])
        print(scipy.sparse.lil_matrix(A[inspectable][6]))
        exit()
